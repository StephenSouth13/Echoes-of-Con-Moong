...
{
    // 1. Kiểm tra chỉ số mảng có hợp lệ không (Tránh lỗi Index Out Of Range)
    if (index < 0 || index >= slots.Length)
        return;

    // 2. Nếu ô đồ này trống hoàn toàn (null) hoặc không có item bên trong
    if (slots[index] == null || slots[index].item == null)
    {
        ClearSelectedItemWindow(); // Tự động ẩn nút và xóa chữ khi bấm ô trống
        return;
    }

    // 3. Gán món đồ được chọn
    ...
    // 5. Bật/Tắt các nút bấm tùy theo loại vật phẩm
    // Đồ tiêu thụ (thức ăn, thuốc)
    useButton.SetActive(selectedItem.item.type == ItemType.Consumable);

    // Đồ trang bị (vũ khí, công cụ)
    bool isEquippable = selectedItem.item.type == ItemType.Equipable;
    bool isEquipped = uiSlots[index].equipped;

    equipButton.SetActive(isEquippable && !isEquipped);
    unequipButton.SetActive(isEquippable && isEquipped);

    // Nút vứt đồ luôn hiện khi có item
